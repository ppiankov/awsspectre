...
AWS resource exposure and misconfiguration scanner.
...
Scans AWS resources for security findings.
...
- Does not remediate or modify AWS resources — scan is read-only
- Does not store findings or manage a findings database
- Does not replace dedicated AWS resources monitoring — point-in-time security audit only
...
```bash
awsspectre scan --output json | jq '.summary'
awsspectre scan --output json | jq '.findings[] | select(.severity == "critical")'
```
